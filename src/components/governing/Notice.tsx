...
const NoticeCard: React.FC<NoticeItem> = ({ title, date, pdfUrl }) => {
    return (
        <div className="flex items-start gap-4 py-3 border-b border-gray-200 last:border-b-0">
            ...
            <div className="flex-1">
                ...
                {/* Optional: Add PDF link if needed */}
                <Link href={pdfUrl} className="text-sm text-indigo-600 hover:underline">
                    View PDF
                </Link>
            </div>
        </div>
...
const Notice: React.FC = () => {
    ...
    useEffect(() => {
        const titleElement = titleRef.current; // Store ref value in a variable
    ...
    }, []); // Empty dependency array is correct since titleRef is stable
    ...
    return (
        <section className="relative bg-white shadow rounded-md overflow-hidden">
            <div className="flex items-center bg-blue-700 text-white">
                ...
                <h3 ref={titleRef} className="text-xl font-semibold ml-4">
                    Notice
                </h3>
            </div>
            <div className="p-4">
